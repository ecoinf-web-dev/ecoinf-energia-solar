<template>
	<header
		class="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 py-3 lg:px-8 bg-gray-900 border-b border-gray-700 shadow-lg backdrop-blur-md transition-all duration-300 ease-in-out">
		<!-- Logo -->
		<div class="shrink-0">
			<NuxtImg
				...
				loading="lazy"
				class="w-auto h-10 lg:h-14"
				format="webp" />
		</div>

		<!-- Mobile Menu Button -->
		<button
			class="relative z-10 p-2 text-white hover:text-yellow-400 transition-colors duration-300 md:hidden"
			...
			<div
				...
				<span
					class="block absolute h-0.5 w-full bg-current transition-all duration-300 ease-in-out origin-left top-0"
		...
		<nav
			...
			<ul
				...
				<li class="w-full md:w-auto">
					<a
						href="#"
						class="block w-full md:w-auto py-3 px-4 md:py-2 md:px-0 text-gray-300 hover:text-yellow-400 font-medium text-sm lg:text-base transition-all duration-300 border-l-4 border-transparent md:border-none hover:border-yellow-400 md:hover:border-none hover:bg-gray-800 md:hover:bg-transparent hover:md:scale-105"
		...
		<div class="shrink-0">
			<button
				class="bg-linear-to-r from-yellow-500 to-yellow-600 text-gray-900 font-bold px-4 py-2 lg:px-6 lg:py-3 text-sm lg:text-base rounded-lg hover:from-yellow-400 hover:to-yellow-500 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-offset-2 focus:ring-offset-gray-900 transform transition-all duration-300 shadow-lg hover:shadow-xl hover:scale-105 hover:-translate-y-0.5 active:scale-95"
				@click="handleQuoteRequest">
				<span class="hidden sm:inline">Fazer Orçamento</span>
